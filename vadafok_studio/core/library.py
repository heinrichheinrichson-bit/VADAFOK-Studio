...
@dataclass
class LibraryItem:
    path: Path; name: str; section: str; category: str; relative: str
    ...
    for root_name in ROOT_FOLDERS:
        ...
        for p in root.rglob("*"):
            if p.is_file() and p.suffix.lower() in IMAGE_EXTS:
                ...
                items.append(LibraryItem(p,p.name,root_name,cat,str(p.relative_to(base))))
    for p in base.glob("*"):
        if p.is_file() and p.suffix.lower() in IMAGE_EXTS:
            items.append(LibraryItem(p,p.name,"Scene Cards","(Root)",str(p.relative_to(base))))
