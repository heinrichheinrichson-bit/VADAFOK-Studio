...
class VadafokStudio(ctk.CTk):
    ...
    def open_sound_favorites_editor(self):
        favorites = [dict(item) for item in self._ensure_sound_favorites()]
        ...
        for index, favorite in enumerate(favorites):
            ...
            ctk.CTkLabel(row, text=f"{index + 1}", width=28, text_color=GOLD).grid(row=0, column=0, rowspan=2, padx=(10, 4), pady=8)
            ctk.CTkEntry(row, textvariable=name_var, placeholder_text=f"Favorit {index + 1}").grid(row=0, column=1, padx=5, pady=(8, 3), sticky="ew")
            ctk.CTkEntry(row, textvariable=file_var, state="readonly").grid(row=1, column=1, padx=5, pady=(3, 8), sticky="ew")

            def browse(slot=index, target=file_var):
                project = str(self.project_folder.get() or "").strip()
                if not project:
                    messagebox.showwarning("Sound-Favoriten", "Bitte zuerst unter Settings einen Projektordner auswählen.", parent=dialog)
                    return
                service = self._sync_sound_service_project()
                sounds_dir = service.sounds_dir
                if sounds_dir is None or not sounds_dir.is_dir():
                    messagebox.showwarning("Sound-Favoriten", f"Der Sound-Ordner wurde nicht gefunden:\n\n{Path(project) / 'Sounds'}", parent=dialog)
                    return
                ...
                if selected:
                    ...
                    if relative is None:
                        messagebox.showerror("Sound-Favoriten", "Bitte eine WAV-Datei innerhalb des Projektordners Sounds auswählen.", parent=dialog)
                        return
                    target.set(relative)

            ctk.CTkButton(row, text="DURCHSUCHEN", width=115, command=browse, fg_color="#333333", hover_color="#444444").grid(row=0, column=2, rowspan=2, padx=(5, 10), pady=8)
        ...
        def save_and_close():
            self.config_data["sound_favorites"] = [
                {
        ...
        ctk.CTkButton(actions, text="ABBRECHEN", command=dialog.destroy, fg_color="#333333", hover_color="#444444").grid(row=0, column=0, padx=(0, 5), sticky="ew")
        ctk.CTkButton(actions, text="SPEICHERN", command=save_and_close, fg_color=GOLD, hover_color=GOLD_DARK, text_color="#111111").grid(row=0, column=1, padx=(5, 0), sticky="ew")
        ...
        try:
            ...
            for idx, action in enumerate(actions, start=1):
                ...
                if action_type == "switch_scene":
                    ...
                    self.director_set_status(
                        "WAITING",
                        f"WAIT {seconds:g} s",
                        f"{idx} / {total}"
                    )
                    self.director_log_add(f"WAIT {seconds:g} s")
                    ...
                    while time.monotonic() < deadline:
                        ...
                        self.director_current_action_var.set(
                    ...
                    self.director_log_add("WAIT finished")
    ...
    def silent_director_normalize_wait_seconds(self):
        raw = str(self.silent_director_wait_seconds.get() or "").strip().replace(",", ".")
        try:
            value = float(raw)
        except Exception:
            value = 5.0
    ...
    def silent_director_update_action_fields(self, *_args):
        action_type = str(self.silent_director_action_type.get() or "").strip()
        frames = getattr(self, "silent_director_dynamic_frames", {}) or {}
    ...
    def show_silent_director_page(self):
        ...
        self.silent_director_actions_frame = ctk.CTkFrame(
            editor,
            fg_color="transparent"
        )
        self.silent_director_actions_frame.grid(
            row=5, column=0, columnspan=2,
            sticky="ew", padx=12, pady=(0, 6)
        )
        self.silent_director_actions_frame.grid_columnconfigure(0, weight=1)
        self.silent_director_render_actions_list()
        ...
        ctk.CTkOptionMenu(
            ...
            variable=self.silent_director_action_type,
            command=self.silent_director_update_action_fields,
            fg_color="#333333",
            button_color="#444444",
            button_hover_color="#555555"
        ...
        ctk.CTkOptionMenu(
            scene_frame,
            values=scene_values,
            variable=self.silent_director_action_scene,
            fg_color="#333333",
            button_color="#444444",
            button_hover_color="#555555"
        ).grid(row=0, column=1, padx=12, pady=6, sticky="ew")
        self.silent_director_dynamic_frames["scene"] = scene_frame
        ...
        ctk.CTkOptionMenu(
            source_frame,
            values=self.silent_director_source_values(),
            variable=self.silent_director_action_source,
            fg_color="#333333",
            button_color="#444444",
            button_hover_color="#555555"
        ).grid(row=0, column=1, padx=12, pady=6, sticky="ew")
        self.silent_director_dynamic_frames["source"] = source_frame
        ...
        ctk.CTkEntry(
            banner_frame,
            textvariable=self.silent_director_action_text,
            placeholder_text="Text for Show Banner action"
        ).grid(row=0, column=1, padx=12, pady=6, sticky="ew")
        self.silent_director_dynamic_frames["banner"] = banner_frame
        ...
        ctk.CTkEntry(
            wait_frame,
            textvariable=self.silent_director_wait_seconds,
            placeholder_text="5"
        ).grid(row=0, column=1, padx=12, pady=6, sticky="ew")
        self.silent_director_dynamic_frames["wait"] = wait_frame
        ...
        ctk.CTkButton(
            button_row,
            textvariable=self.silent_director_action_button_text,
            height=42,
            fg_color=GOLD,
            text_color="#111111",
            hover_color=GOLD_DARK,
            command=self.silent_director_add_action
        ...
        ctk.CTkButton(
            ...
            fg_color="#333333",
            hover_color="#444444",
            command=self.silent_director_cancel_action_edit
