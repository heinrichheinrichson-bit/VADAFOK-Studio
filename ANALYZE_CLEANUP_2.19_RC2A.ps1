...
$ReportDir = Join-Path $ProjectRoot "docs\cleanup_reports\2.19_RC2A"
...
$files = Get-ChildItem -LiteralPath $ProjectRoot -File -Recurse -Force | Where-Object {
    ...
    if ($relative -like "docs\cleanup_reports\2.19_RC2A\*") {
        $excluded = $true
    }
...
$lines.Add("# VADAFOK Studio – Cleanup Analyzer 2.19 RC2A")
$lines.Add("")
