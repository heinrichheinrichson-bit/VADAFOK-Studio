...
class SettingsPageModularizationTests(unittest.TestCase):
    def test_settings_page_is_owned_by_settings_module(self):
    ...
    def test_settings_persistence_is_owned_by_controller(self):
        app_source = APP_PATH.read_text(encoding="utf-8")
        controller_source = CONTROLLER_PATH.read_text(encoding="utf-8")
    ...
    def test_live_card_voice_still_targets_quick_caption(self):
        page_source = PAGE_PATH.read_text(encoding="utf-8")
        app_source = APP_PATH.read_text(encoding="utf-8")
