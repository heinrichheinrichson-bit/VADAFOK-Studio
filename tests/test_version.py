...
def test_current_version():
    assert VERSION == "2.29.0.10"


def test_runtime_version_constants_are_available():
    assert PRODUCT_NAME == "VADAFOK Studio"
    assert APP_TITLE == "VADAFOK Studio 2.29.0.10"
    assert SIDEBAR_VERSION == "Studio 2.29.0.10"
    assert APP_USER_MODEL_ID == "VADAFOK.Studio.2.29.0.10"
