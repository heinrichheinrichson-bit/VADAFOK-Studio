...
class VersionTests(unittest.TestCase):
    def test_current_version(self):
        assert VERSION == "2.29.0.20"


    def test_runtime_version_constants_are_available(self):
        assert PRODUCT_NAME == "VADAFOK Studio"
        assert APP_TITLE == "VADAFOK Studio 2.29.0.20"
        assert SIDEBAR_VERSION == "Studio 2.29.0.20"
        assert APP_USER_MODEL_ID == "VADAFOK.Studio.2.29.0.20"
