...
def main():
    print("VADAFOK Studio 2.24.1 - Foundation - Card Creator Controller")
    print("=" * 68)
    suite = unittest.defaultTestLoader.discover(
        str(ROOT / "tests"), pattern="test_*.py", top_level_dir=str(ROOT)
    )
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    compile_ok = compileall.compile_dir(str(ROOT / "vadafok_studio"), quiet=1, force=True)
    print()
    ...
    if result.wasSuccessful() and compile_ok:
        print("[FERTIG] Alle 2.24.1-Tests wurden bestanden.")
        return 0
    return 1
